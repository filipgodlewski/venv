...
function _venv::delete {
  ...
  [[ ${#@} -eq 0 ]] && {echo "No venvs provided."; return 1}
  ...
  for venv in $@; do
    ...
    if [[ ${#lines:#} -gt 1 && ${flags[(Ie)--force]} -eq 0 ]]; then
      echo "The venv you want to delete is used by more than 1 project:"
      echo ${(F@)lines:#}
      echo
    ...
    rm -rf $venv_path
    echo "Deleted venv named '$venv' which was stored under '$venv_path'"
  done
}
